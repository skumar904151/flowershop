...
This repository contains a monorepo that hosts the customer storefront, backend API, and administrative dashboard for the Flower Shop platform. Each application lives in its own workspace (`frontend`, `backend`, `admin`) and is orchestrated with pnpm. See [`docs/architecture.md`](docs/architecture.md) for implementation details and next steps.

🌸 **Frontend (`user.flowershop.com`)

Tech Stack: React (Next.js) + Tailwind CSS for modern, mobile-responsive UI
